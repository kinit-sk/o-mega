...
class STS_ExplainWrapper(torch.nn.Module):
    ...
    """
    def __init__(self, model: RepresentationModel) -> None:
        super().__init__()
        self.model = model
...
class ExplanationExecuter:
    ...
    def explain_batch(
        ...
        post_explanation = self.postprocess_explanations(post_explanation)
        if return_one_explanation == False:
            ...
            claim_explanation = self.postprocess_explanations(claim_explanation)
    ...
    def _vizualize_pair(
        ...
        # TODO sum relevance of real tokens to interpretable tokens
...
def compare_multiple_explanation_methods(
    explain_executors: list[ExplanationExecuter], post: str, 
    claim: str, additional_attribution_kwargs: list[dict] = None, 
    method_names: list[str] = None, visualize: bool = True
    ...
    for expl_it, (explain_wrapper, method_name) in enumerate(zip(explain_executors, method_names)):
        ...
        # post_tokens = [
        #     explain_wrapper.tokenizer.convert_ids_to_tokens(input_ids) 
        #     for input_ids in encodings[0]["input_encodings"]["input_ids"]
        # ]
        # claim_tokens = [
        #     explain_wrapper.tokenizer.convert_ids_to_tokens(input_ids) 
        #     for input_ids in encodings[1]["input_encodings"]["input_ids"]
        # ]
        # tokens = (post_tokens, claim_tokens)
        ...
        if  visualize == True:
            ...
            all_explanations[method_name] = (explanations[0],explanations[1])
    ...
    print(f"\t[CLAIM]: {claim}")
    methods_values=[]
    for expl_it, type_of_string in enumerate(["[POST]", "[CLAIM]"]):
